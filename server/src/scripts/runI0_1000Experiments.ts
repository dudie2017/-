/**
 * 铁矿石(I0) 1000次完全不同实验：24维方法论配方空间探测
 *
 ...
 *
 * 运行：cd server && npx tsx src/scripts/runI0_1000Experiments.ts
 * 输出：src/data/I0_1000Experiments.json
 */
...
function buildBlackSwanFilter(mode: 'none' | 'riskOff' | 'full'): NewsFilter | undefined {
  ...
  try {
    ...
    console.warn('[I0] 黑天鹅检测异常，回退为空过滤器:', (e as Error).message);
  }
  return {
    mode,
...
const SEED = 20260817; // I0 专用种子
const OUT_FILE = 'I0_1000Experiments.json';
...
async function main() {
  console.log('========== I0 铁矿石 1000 次完全不同实验（24 维方法论空间） ==========');
  console.log('1/6 加载数据与理论基准...');
  const bars = loadBars(CODE);
  console.log(`I0 K线 ${bars.length} 根（${bars[0]?.date} ~ ${bars[bars.length - 1]?.date}）`);
  ...
  console.log('3/6 运行基线（I0 铁矿石寻优配方）...');
  // I0 铁矿石生产参数（高波动型：中宽止损 + 中等持仓 + 波动率过滤 + 3x10 熔断）
  // 来源: longOptParams I0 2.85/3.98/39, shortOptParams I0 2.47/5.99/38
