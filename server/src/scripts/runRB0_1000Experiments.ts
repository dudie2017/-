/**
 * 螺纹钢(RB0) 1000次完全不同实验：24维方法论配方空间探测
 *
 ...
 *
 * 运行：cd server && npx tsx src/scripts/runRB0_1000Experiments.ts
 * 输出：src/data/RB0_1000Experiments.json
 */
...
function buildBlackSwanFilter(mode: 'none' | 'riskOff' | 'full'): NewsFilter | undefined {
  ...
  try {
    ...
    console.warn('[RB0] 黑天鹅检测异常，回退为空过滤器:', (e as Error).message);
  }
  return {
    mode,
...
// ============ 生产基线参数 ============
function getBaseLong() { return LONG_REFINED_PARAMS[CODE] ?? LONG_OPT_PARAMS[CODE] ?? { stopAtrMult: 2.77, targetAtrMult: 5.45, maxHoldDays: 36, cooldownBars: 4, trendFilter: false, minSignalGrade: 'L1' }; }
function getBaseShort() { return SHORT_OPT_PARAMS[CODE] ?? { stopAtrMult: 2.85, targetAtrMult: 3.98, maxHoldDays: 39, cooldownBars: 1, trendFilter: false, minSignalGrade: 'L2' }; }
...
function crossCompareAll(cu0Results: ExpResult[]) {
  const otherCodes = ['SC0', 'CU0', 'LH0', 'JM0', 'M0', 'AG0', 'RU0'];
...
const SEED = 20260816; // RB0 专用种子
const OUT_FILE = 'RB0_1000Experiments.json';
...
async function main() {
  console.log('========== RB0 螺纹钢 1000 次完全不同实验（24 维方法论空间） ==========');
  console.log('1/6 加载数据与理论基准...');
  const bars = loadBars(CODE);
  console.log(`RB0 K线 ${bars.length} 根（${bars[0]?.date} ~ ${bars[bars.length - 1]?.date}）`);
  ...
  console.log('3/6 运行基线（RB0 螺纹钢寻优配方）...');
  // RB0 螺纹钢生产参数（慢趋势型：宽止损 + 长持仓 + 无波动率过滤 + 5x20 熔断）
  // 来源: longOptParams RB0 2.77/5.45/36, shortOptParams RB0 2.85/3.98/39
  const baseRecipe: Record<string, number | string | boolean> = {
    ...
    stopAtrMult: 2.77, targetAtrMult: 5.45, maxHoldDays: 36, minRR: 1.0,
