/**
 * 橡胶(RU0) 1000次完全不同实验：24维方法论配方空间探测
 *
 ...
 *
 * 运行：cd server && npx tsx src/scripts/runRU0_1000Experiments.ts
 * 输出：src/data/RU0_1000Experiments.json
 */
...
function applyFeeMult(trades: TradeLike[], mult: number): TradeLike[] {
  ...
  const multiplier = 10; // RU0 橡胶合约乘数（吨/手）；LH0 为 16、JM0 为 60、M0 为 10、AG0 为 15
  return trades.map((t) => {
...
/** 五方交叉对比：读取 LH0/JM0/M0/AG0 四个 JSON，对比脆弱点/最优配方/方向模式/捕获异常 */
function crossCompareAll(ru0Results: ExpResult[]) {
  ...
  const ag0Path = path.join(process.cwd(), 'src', 'data', 'AG0_1000Experiments.json');
  let lh0: any = null;
  let jm0: any = null;
  let m0: any = null;
  let ag0: any = null;
  ...
  try {
    ag0 = JSON.parse(fs.readFileSync(ag0Path, 'utf8'));
  } catch {
    ag0 = null;
  }
  const ru0Top = [...ru0Results].sort((a, b) => b.stats.totalPnl - a.stats.totalPnl)[0];
  const ru0TopDir = ru0Top?.recipe.directionMode ?? '';
  ...
  const ag0TopDir = ag0?.topPnl?.[0]?.recipe?.directionMode ?? null;
  const ru0LongOver = ru0Results.filter((r) => r.stats.longCapture > 1.0).length / ru0Results.length;
  ...
  const m0LongOver = m0?.fullResults
    ...
    : null;
  const ag0LongOver = ag0?.fullResults
    ? ag0.fullResults.filter((r: any) => r.stats.longCapture > 1.0).length / ag0.fullResults.length
    : null;
  const ru0ShortOver = ru0Results.filter((r) => r.stats.shortCapture > 1.0).length / ru0Results.length;
  ...
  const m0ShortOver = m0?.fullResults
    ...
    : null;
  const ag0ShortOver = ag0?.fullResults
    ? ag0.fullResults.filter((r: any) => r.stats.shortCapture > 1.0).length / ag0.fullResults.length
    : null;
  return {
    ...
    ag0Experiments: ag0?.meta?.experiments ?? null,
    ag0DateRange: ag0?.meta?.dateRange ?? null,
    topPnlDirectionMode: { ru0: ru0TopDir, lh0: lh0TopDir, jm0: jm0TopDir, m0: m0TopDir, ag0: ag0TopDir },
    longCaptureOver100Ratio: {
      ru0: +ru0LongOver.toFixed(4),
      ...
      ag0: ag0LongOver !== null ? +ag0LongOver.toFixed(4) : null,
    },
    shortCaptureOver100Ratio: {
      ru0: +ru0ShortOver.toFixed(4),
      ...
      ag0: ag0ShortOver !== null ? +ag0ShortOver.toFixed(4) : null,
    },
    ...
    ag0TopFragility: ag0?.fragility?.topFactors?.slice(0, 5) ?? [],
    ...
    ag0TopVarianceDim: ag0?.varianceDecomposition?.totalPnl?.[0]?.dimension ?? null,
  };
}
...
const SEED = 20260811;
const OUT_FILE = 'RU0_1000Experiments.json';
...
async function main() {
  console.log('========== RU0 橡胶 1000 次完全不同实验（24 维方法论空间） ==========');
  console.log('1/6 加载数据与理论基准...');
  const bars = loadBars(CODE);
  console.log(`RU0 K线 ${bars.length} 根（${bars[0]?.date} ~ ${bars[bars.length - 1]?.date}）`);
  ...
  // 当前生产形态：多空分离参数（LONG_OPT_PARAMS['RU0'] + SHORT_OPT_PARAMS['RU0']）
  // 注：RU0 多空生产参数完全相同（2.85/3.98/39/L2/cooldown1），directionMode='split' 时引擎按分离读取
  ...
  const baseRecipe: Record<string, number | string | boolean> = {
    minSignalGrade: 'L2', trendFilter: false, cooldownBars: 1, edgeLookback: 50,
    allowRangeTrading: true, equationMode: 'none', pThreshold: 0.35, softEquationMul: 0.7,
    stopAtrMult: 2.85, targetAtrMult: 3.98, maxHoldDays: 39, minRR: 1.0,
    maxPositionPct: 0.2, directionMode: 'split', dataWindow: 'full',
    nonGreenMul: 1.5, counterCampMul: 0.5, campWindow: 21, feeMult: 3.0,
    startCapital: 500000, bsMode: 'riskOff',
  ...
  // ============ RU0 针对性分析（做多持仓周期 / 方向模式 / 熔断 / 五方交叉） ============
  console.log('--- RU0 针对性分析 ---');
  ...
  console.log('五方交叉对比:');
  console.log(`  最优方向模式: RU0=${cross.topPnlDirectionMode.ru0} LH0=${cross.topPnlDirectionMode.lh0} JM0=${cross.topPnlDirectionMode.jm0} M0=${cross.topPnlDirectionMode.m0} AG0=${cross.topPnlDirectionMode.ag0}`);
  console.log(`  做多捕获>100%占比: RU0=${(cross.longCaptureOver100Ratio.ru0 * 100).toFixed(1)}% LH0=${cross.longCaptureOver100Ratio.lh0 !== null ? (cross.longCaptureOver100Ratio.lh0 * 100).toFixed(1) + '%' : 'N/A'} JM0=${cross.longCaptureOver100Ratio.jm0 !== null ? (cross.longCaptureOver100Ratio.jm0 * 100).toFixed(1) + '%' : 'N/A'} M0=${cross.longCaptureOver100Ratio.m0 !== null ? (cross.longCaptureOver100Ratio.m0 * 100).toFixed(1) + '%' : 'N/A'} AG0=${cross.longCaptureOver100Ratio.ag0 !== null ? (cross.longCaptureOver100Ratio.ag0 * 100).toFixed(1) + '%' : 'N/A'}`);
  console.log(`  做空捕获>100%占比: RU0=${(cross.shortCaptureOver100Ratio.ru0 * 100).toFixed(1)}% LH0=${cross.shortCaptureOver100Ratio.lh0 !== null ? (cross.shortCaptureOver100Ratio.lh0 * 100).toFixed(1) + '%' : 'N/A'} JM0=${cross.shortCaptureOver100Ratio.jm0 !== null ? (cross.shortCaptureOver100Ratio.jm0 * 100).toFixed(1) + '%' : 'N/A'} M0=${cross.shortCaptureOver100Ratio.m0 !== null ? (cross.shortCaptureOver100Ratio.m0 * 100).toFixed(1) + '%' : 'N/A'} AG0=${cross.shortCaptureOver100Ratio.ag0 !== null ? (cross.shortCaptureOver100Ratio.ag0 * 100).toFixed(1) + '%' : 'N/A'}`);
