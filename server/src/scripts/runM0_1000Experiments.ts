/**
 * 豆粕(M0) 1000次完全不同实验：24维方法论配方空间探测
 *
 ...
 *
 * 运行：cd server && npx tsx src/scripts/runM0_1000Experiments.ts
 * 输出：src/data/M0_1000Experiments.json
 */
...
function applyFeeMult(trades: TradeLike[], mult: number): TradeLike[] {
  ...
  const multiplier = 10; // M0 豆粕合约乘数（吨/手）；LH0 为 16、JM0 为 60、AG0 为 15
  return trades.map((t) => {
...
/** 与 LH0/JM0 交叉对比：读取 LH0_1000Experiments.json 和 JM0_1000Experiments.json，对比脆弱点/最优配方/方向模式 */
function crossCompareLH0JM0(m0Results: ExpResult[]) {
  ...
  const m0Top = [...m0Results].sort((a, b) => b.stats.totalPnl - a.stats.totalPnl)[0];
  const m0TopDir = m0Top?.recipe.directionMode ?? '';
  const lh0TopDir = lh0?.topPnl?.[0]?.recipe?.directionMode ?? null;
  const jm0TopDir = jm0?.topPnl?.[0]?.recipe?.directionMode ?? null;
  const m0LongOver = m0Results.filter((r) => r.stats.longCapture > 1.0).length / m0Results.length;
  ...
  return {
    ...
    topPnlDirectionMode: { m0: m0TopDir, lh0: lh0TopDir, jm0: jm0TopDir },
    longCaptureOver100Ratio: {
      m0: +m0LongOver.toFixed(4),
...
const SEED = 20260811;
const OUT_FILE = 'M0_1000Experiments.json';
...
async function main() {
  console.log('========== M0 豆粕 1000 次完全不同实验（24 维方法论空间） ==========');
  console.log('1/6 加载数据与理论基准...');
  const bars = loadBars(CODE);
  console.log(`M0 K线 ${bars.length} 根（${bars[0]?.date} ~ ${bars[bars.length - 1]?.date}）`);
  ...
  // 当前生产形态：多空分离参数（LONG_OPT_PARAMS['M0'] + SHORT_OPT_PARAMS['M0']）
  ...
  const baseRecipe: Record<string, number | string | boolean> = {
    minSignalGrade: 'L1', trendFilter: false, cooldownBars: 5, edgeLookback: 70,
    allowRangeTrading: true, equationMode: 'soft', pThreshold: 0.45, softEquationMul: 0.5,
    stopAtrMult: 2.69, targetAtrMult: 5.22, maxHoldDays: 38, minRR: 1.0,
  ...
  // ============ M0 针对性分析（做多持仓周期 / 方向模式 / 熔断 / LH0+JM0三方交叉） ============
  console.log('--- M0 针对性分析 ---');
  ...
  const cross = crossCompareLH0JM0(results);
  if (cross) {
    console.log('LH0/JM0 三方交叉对比:');
    console.log(`  方向模式最优: M0=${cross.topPnlDirectionMode.m0} JM0=${cross.topPnlDirectionMode.jm0} LH0=${cross.topPnlDirectionMode.lh0}`);
    console.log(`  做多捕获>100%占比: M0=${(cross.longCaptureOver100Ratio.m0 * 100).toFixed(1)}% JM0=${cross.longCaptureOver100Ratio.jm0 !== null ? (cross.longCaptureOver100Ratio.jm0 * 100).toFixed(1) + '%' : 'N/A'} LH0=${cross.longCaptureOver100Ratio.lh0 !== null ? (cross.longCaptureOver100Ratio.lh0 * 100).toFixed(1) + '%' : 'N/A'}`);
  }
