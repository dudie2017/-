/**
 * 铜(CU0) 1000次完全不同实验：24维方法论配方空间探测
 *
 ...
 *
 * 运行：cd server && npx tsx src/scripts/runCU0_1000Experiments.ts
 * 输出：src/data/CU0_1000Experiments.json
 */
...
function buildBlackSwanFilter(mode: 'none' | 'riskOff' | 'full'): NewsFilter | undefined {
  ...
  try {
    ...
    console.warn('[CU0] 黑天鹅检测异常，回退为空过滤器:', (e as Error).message);
  }
  return {
    mode,
...
const SEED = 20260814; // CU0 专用种子
const OUT_FILE = 'CU0_1000Experiments.json';
...
async function main() {
  console.log('========== CU0 铜 1000 次完全不同实验（24 维方法论空间） ==========');
  console.log('1/6 加载数据与理论基准...');
  const bars = loadBars(CODE);
  console.log(`CU0 K线 ${bars.length} 根（${bars[0]?.date} ~ ${bars[bars.length - 1]?.date}）`);
  ...
  console.log('3/6 运行基线（CU0 铜寻优配方）...');
  // CU0 铜生产参数（慢趋势型：宽止损 + 长持仓 + 无波动率过滤 + 5x20 熔断）
  // 来源: longOptParams CU0 2.3/5.03/39, shortOptParams CU0 2.69/5.22/38
