/**
 * 焦煤(JM0) 1000次完全不同实验：24维方法论配方空间探测
 *
 ...
 *
 * 运行：cd server && npx tsx src/scripts/runJM0_1000Experiments.ts
 * 输出：src/data/JM0_1000Experiments.json
 */
...
function applyFeeMult(trades: TradeLike[], mult: number): TradeLike[] {
  ...
  const multiplier = 60; // JM0 焦煤合约乘数（吨/手）；LH0 为 16
  return trades.map((t) => {
...
/** 做空持仓周期分析：按 maxHoldDays 分组，看做空收益/捕获率是否依赖长持仓 */
function analyzeShortHold(results: ExpResult[]): Array<Record<string, number | string>> {
  const groups = new Map<string, Array<{ shortPnl: number; shortCap: number; shortTrades: number }>>();
  for (const r of results) {
    const rec = r.recipe;
    // 只看含做空的实验（directionMode 不是 longOnly）
    if (String(rec.directionMode) === 'longOnly') continue;
    const hold = String(rec.maxHoldDays);
    if (!groups.has(hold)) groups.set(hold, []);
    groups.get(hold)!.push({
      shortPnl: r.stats.shortPnl,
      shortCap: r.stats.shortCapture,
      shortTrades: r.stats.shortTrades,
    });
  }
  const out: Array<Record<string, number | string>> = [];
  for (const [hold, arr] of groups) {
    const n = arr.length;
    const avgPnl = arr.reduce((s, x) => s + x.shortPnl, 0) / n;
    const avgCap = arr.reduce((s, x) => s + x.shortCap, 0) / n;
    const avgTrades = arr.reduce((s, x) => s + x.shortTrades, 0) / n;
    const posRatio = arr.filter((x) => x.shortPnl > 0).length / n;
    out.push({
      maxHoldDays: hold,
      n,
      avgShortPnl: +avgPnl.toFixed(0),
      avgShortCapture: +avgCap.toFixed(4),
      avgShortTrades: +avgTrades.toFixed(0),
      shortProfitableRatio: +posRatio.toFixed(4),
    });
  }
...
/** 方向模式对比：split vs both vs longOnly vs shortOnly（做空捕获率异常是否源于方向模式） */
function analyzeDirectionModes(results: ExpResult[]): Array<Record<string, number | string>> {
  ...
  for (const [dm, arr] of groups) {
    ...
    const avgShortPnl = arr.reduce((s, r) => s + r.stats.shortPnl, 0) / n;
    const shortOver100 = arr.filter((r) => r.stats.shortCapture > 1.0).length;
    out.push({
      directionMode: dm,
      n,
      ...
      avgShortPnl: +avgShortPnl.toFixed(0),
      shortCaptureOver100Ratio: +(shortOver100 / n).toFixed(4),
    });
  }
...
/** 与 LH0 交叉对比：读取 LH0_1000Experiments.json，对比脆弱点/最优配方/方向模式 */
function crossCompareLH0(jm0Results: ExpResult[]) {
  ...
  const jm0Top = [...jm0Results].sort((a, b) => b.stats.totalPnl - a.stats.totalPnl)[0];
  const jm0TopDir = jm0Top?.recipe.directionMode ?? '';
  const lh0TopDir = lh0.topPnl?.[0]?.recipe?.directionMode ?? '';
  const jm0ShortOver = jm0Results.filter((r) => r.stats.shortCapture > 1.0).length / jm0Results.length;
  ...
  return {
    ...
    topPnlDirectionMode: { jm0: jm0TopDir, lh0: lh0TopDir },
    shortCaptureOver100Ratio: {
      jm0: +jm0ShortOver.toFixed(4),
...
const SEED = 20260811;
const OUT_FILE = 'JM0_1000Experiments.json';
...
async function main() {
  console.log('========== JM0 焦煤 1000 次完全不同实验（24 维方法论空间） ==========');
  console.log('1/6 加载数据与理论基准...');
  const bars = loadBars(CODE);
  console.log(`JM0 K线 ${bars.length} 根（${bars[0]?.date} ~ ${bars[bars.length - 1]?.date}）`);
  ...
  // 当前生产形态：多空分离参数（LONG_OPT_PARAMS['JM0'] + SHORT_OPT_PARAMS['JM0']）
  ...
  // 若设 true 会多出区间交易（611→1037笔），基线失真
  const baseRecipe: Record<string, number | string | boolean> = {
    minSignalGrade: 'L1', trendFilter: false, cooldownBars: 0, edgeLookback: 50,
    allowRangeTrading: true, equationMode: 'none', pThreshold: 0.35, softEquationMul: 0.7,
    stopAtrMult: 1.55, targetAtrMult: 5.52, maxHoldDays: 22, minRR: 1.0,
  ...
  // ============ JM0 针对性分析（做空持仓周期 / 方向模式 / 熔断 / LH0交叉） ============
  console.log('--- JM0 针对性分析 ---');
  const shortHold = analyzeShortHold(results);
  console.log('做空持仓周期:');
  shortHold.forEach((s) =>
    console.log(`  hold=${s.maxHoldDays} n=${s.n} 做空收益均值${(Number(s.avgShortPnl)).toLocaleString()} 捕获${(Number(s.avgShortCapture) * 100).toFixed(1)}% 盈利组占比${(Number(s.shortProfitableRatio) * 100).toFixed(0)}%`),
  );
  const dirModes = analyzeDirectionModes(results);
  console.log('方向模式对比:');
  dirModes.forEach((d) =>
    console.log(`  ${d.directionMode} n=${d.n} 收益${(Number(d.avgPnl)).toLocaleString()} 胜率${(Number(d.avgWinRate) * 100).toFixed(1)}% 回撤${(Number(d.avgMaxDD) * 100).toFixed(1)}% 多捕获${(Number(d.avgLongCapture) * 100).toFixed(1)}% 空捕获${(Number(d.avgShortCapture) * 100).toFixed(1)}% 空>100%占比${(Number(d.shortCaptureOver100Ratio) * 100).toFixed(0)}%`),
  );
  ...
  const cross = crossCompareLH0(results);
  if (cross) {
    console.log('LH0 交叉对比:');
    console.log(`  方向模式最优: JM0=${cross.topPnlDirectionMode.jm0} LH0=${cross.topPnlDirectionMode.lh0}`);
    console.log(`  做空捕获>100%占比: JM0=${(cross.shortCaptureOver100Ratio.jm0 * 100).toFixed(1)}% LH0=${cross.shortCaptureOver100Ratio.lh0 !== null ? (cross.shortCaptureOver100Ratio.lh0 * 100).toFixed(1) + '%' : 'N/A'}`);
  }
