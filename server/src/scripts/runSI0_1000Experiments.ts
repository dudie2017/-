/**
 * 工业硅(SI0) 1000次完全不同实验：24维方法论配方空间探测
 *
 ...
 *
 * 运行：cd server && npx tsx src/scripts/runSI0_1000Experiments.ts
 * 输出：src/data/SI0_1000Experiments.json
 *
...
function applyFeeMult(trades: TradeLike[], mult: number): TradeLike[] {
  ...
  const multiplier = 5; // SI 工业硅合约乘数（5吨/手）
  return trades.map((t) => {
...
const SEED = 20260910;
const OUT_FILE = 'SI0_1000Experiments.json';
...
async function main() {
  console.log('========== SI0 工业硅 1000 次完全不同实验（24 维方法论空间） ==========');
  console.log('1/6 加载数据与理论基准...');
  const bars = loadBars(CODE);
  console.log(`SI0 K线 ${bars.length} 根（${bars[0]?.date} ~ ${bars[bars.length - 1]?.date}）`);
