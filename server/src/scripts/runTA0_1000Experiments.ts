/**
 * PTA(TA0) 1000次完全不同实验：24维方法论配方空间探测
 *
 ...
 *
 * 运行：cd server && npx tsx src/scripts/runTA0_1000Experiments.ts
 * 输出：src/data/TA0_1000Experiments.json
 */
...
function buildBlackSwanFilter(mode: 'none' | 'riskOff' | 'full'): NewsFilter | undefined {
  ...
  try {
    ...
    console.warn('[TA0] 黑天鹅检测异常，回退为空过滤器:', (e as Error).message);
  }
  return {
    mode,
...
const SEED = 20260824; // TA0 专用种子
const OUT_FILE = 'TA0_1000Experiments.json';
...
async function main() {
  console.log('========== TA0 PTA 1000 次完全不同实验（24 维方法论空间） ==========');
  console.log('1/6 加载数据与理论基准...');
  const bars = loadBars(CODE);
  console.log(`TA0 K线 ${bars.length} 根（${bars[0]?.date} ~ ${bars[bars.length - 1]?.date}）`);
  ...
  console.log('3/6 运行基线（TA0 PTA寻优配方）...');
  // TA0 PTA生产参数（与 RU0 同型：快趋势 + 3x10 熔断 + atr2xClear 过滤 + 5pct 日亏损）
  // 来源: longOptParams TA0 2.63/5.55/38, shortOptParams TA0 2.77/5.45/36
  const baseRecipe: Record<string, number | string | boolean> = {
    minSignalGrade: 'L2', trendFilter: false, cooldownBars: 5, edgeLookback: 70,
    allowRangeTrading: true, equationMode: 'none', pThreshold: 0.35, softEquationMul: 0.7,
    stopAtrMult: 2.63, targetAtrMult: 5.55, maxHoldDays: 38, minRR: 1.0,
