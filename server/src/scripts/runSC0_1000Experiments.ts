/**
 * 原油(SC0) 1000次完全不同实验：24维方法论配方空间探测
 *
 ...
 *
 * 运行：cd server && npx tsx src/scripts/runSC0_1000Experiments.ts
 * 输出：src/data/SC0_1000Experiments.json
 */
...
function buildBlackSwanFilter(mode: 'none' | 'riskOff' | 'full'): NewsFilter | undefined {
  ...
  try {
    ...
    console.warn('[SC0] 黑天鹅检测异常，回退为空过滤器:', (e as Error).message);
  }
  return {
    mode,
...
// ============ 生产基线参数 ============
function getBaseLong() { return LONG_REFINED_PARAMS[CODE] ?? LONG_OPT_PARAMS[CODE] ?? { stopAtrMult: 2.45, targetAtrMult: 2.85, maxHoldDays: 40, cooldownBars: 4, trendFilter: false, minSignalGrade: 'L1' }; }
function getBaseShort() { return SHORT_OPT_PARAMS[CODE] ?? { stopAtrMult: 2.07, targetAtrMult: 4.49, maxHoldDays: 28, cooldownBars: 1, trendFilter: false, minSignalGrade: 'L1' }; }
...
/** 六方交叉对比：读取 LH0/JM0/M0/AG0/RU0 五个 JSON，对比脆弱点/最优配方/方向模式/捕获异常 */
function crossCompareAll(sc0Results: ExpResult[]) {
  const otherCodes = ['LH0', 'JM0', 'M0', 'AG0', 'RU0'];
  ...
  const sc0Top = [...sc0Results].sort((a, b) => b.stats.totalPnl - a.stats.totalPnl)[0];
  const sc0TopDir = String(sc0Top?.recipe.directionMode ?? '');

  const topPnlDirectionMode: Record<string, string | null> = { sc0: sc0TopDir };
  const longCaptureOver100Ratio: Record<string, number> = {
    sc0: +(sc0Results.filter((r) => r.stats.longCapture > 1.0).length / sc0Results.length).toFixed(4),
  };
  const shortCaptureOver100Ratio: Record<string, number> = {
    sc0: +(sc0Results.filter((r) => r.stats.shortCapture > 1.0).length / sc0Results.length).toFixed(4),
  };
...
const SEED = 20260813; // SC0 专用种子
const OUT_FILE = 'SC0_1000Experiments.json';
...
async function main() {
  console.log('========== SC0 原油 1000 次完全不同实验（24 维方法论空间） ==========');
  console.log('1/6 加载数据与理论基准...');
  const bars = loadBars(CODE);
  console.log(`SC0 K线 ${bars.length} 根（${bars[0]?.date} ~ ${bars[bars.length - 1]?.date}）`);
  ...
  console.log('3/6 运行基线（SC0 原油寻优配方）...');
  // SC0 生产参数升级为 TOP1 寻优配方（高波动型：紧止损 + soft 等式 + 区间 + 波动率过滤 + 日熔断 + 3x10 熔断）
  // 来源: SC0_1000Experiments.json topComposite[0]，仓位 0.3→0.25 稳健化，熔断 off→3x10（脆弱率 49%→30%）
  const baseRecipe: Record<string, number | string | boolean> = {
    minSignalGrade: 'L1', trendFilter: false, cooldownBars: 4, edgeLookback: 70,
    allowRangeTrading: true, equationMode: 'soft', pThreshold: 0.4, softEquationMul: 0.7,
    stopAtrMult: 2.45, targetAtrMult: 2.85, maxHoldDays: 40, minRR: 1.0,
    maxPositionPct: 0.3, directionMode: 'split', dataWindow: 'full',
