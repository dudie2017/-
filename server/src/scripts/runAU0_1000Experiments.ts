/**
 * 黄金(AU0) 1000次完全不同实验：24维方法论配方空间探测
 *
 ...
 *
 * 运行：cd server && npx tsx src/scripts/runAU0_1000Experiments.ts
 * 输出：src/data/AU0_1000Experiments.json
 */
...
function buildBlackSwanFilter(mode: 'none' | 'riskOff' | 'full'): NewsFilter | undefined {
  ...
  try {
    ...
    console.warn('[AU0] 黑天鹅检测异常，回退为空过滤器:', (e as Error).message);
  }
  return {
    mode,
...
const SEED = 20260815; // AU0 专用种子
const OUT_FILE = 'AU0_1000Experiments.json';
...
async function main() {
  console.log('========== AU0 黄金 1000 次完全不同实验（24 维方法论空间） ==========');
  console.log('1/6 加载数据与理论基准...');
  const bars = loadBars(CODE);
  console.log(`AU0 K线 ${bars.length} 根（${bars[0]?.date} ~ ${bars[bars.length - 1]?.date}）`);
  ...
  console.log('3/6 运行基线（AU0 黄金寻优配方）...');
  // AU0 黄金生产参数（与 RU0 同型：快趋势 + 3x10 熔断 + atr2xClear 过滤 + 5pct 日亏损）
  // 来源: longOptParams AU0 2.85/3.98/39, shortOptParams AU0 2.85/3.98/39
