/**
 * 棕榈油(P0) 1000次完全不同实验：24维方法论配方空间探测
 *
 ...
 *
 * 运行：cd server && npx tsx src/scripts/runP0_1000Experiments.ts
 * 输出：src/data/P0_1000Experiments.json
 */
...
function buildBlackSwanFilter(mode: 'none' | 'riskOff' | 'full'): NewsFilter | undefined {
  ...
  try {
    ...
    console.warn('[P0] 黑天鹅检测异常，回退为空过滤器:', (e as Error).message);
  }
  return {
    mode,
...
const SEED = 20260823; // P0 专用种子
const OUT_FILE = 'P0_1000Experiments.json';
...
async function main() {
  console.log('========== P0 棕榈油 1000 次完全不同实验（24 维方法论空间） ==========');
  console.log('1/6 加载数据与理论基准...');
  const bars = loadBars(CODE);
  console.log(`P0 K线 ${bars.length} 根（${bars[0]?.date} ~ ${bars[bars.length - 1]?.date}）`);
  ...
  console.log('3/6 运行基线（P0 棕榈油寻优配方）...');
  // P0 棕榈油生产参数（与 RU0 同型：快趋势 + 3x10 熔断 + atr2xClear 过滤 + 5pct 日亏损）
  // 来源: longOptParams P0 2.89/4.44/32, shortOptParams P0 2.29/4.43/39
  const baseRecipe: Record<string, number | string | boolean> = {
    minSignalGrade: 'L1', trendFilter: false, cooldownBars: 2, edgeLookback: 70,
    allowRangeTrading: true, equationMode: 'none', pThreshold: 0.35, softEquationMul: 0.7,
    stopAtrMult: 2.89, targetAtrMult: 4.44, maxHoldDays: 32, minRR: 1.0,
