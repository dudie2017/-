/**
 * 豆油(Y0) 1000次完全不同实验：24维方法论配方空间探测
 *
 ...
 *
 * 运行：cd server && npx tsx src/scripts/runY0_1000Experiments.ts
 * 输出：src/data/Y0_1000Experiments.json
 */
...
function buildBlackSwanFilter(mode: 'none' | 'riskOff' | 'full'): NewsFilter | undefined {
  ...
  try {
    ...
    console.warn('[Y0] 黑天鹅检测异常，回退为空过滤器:', (e as Error).message);
  }
  return {
    mode,
...
const SEED = 20260821; // Y0 专用种子
const OUT_FILE = 'Y0_1000Experiments.json';
...
async function main() {
  console.log('========== Y0 豆油 1000 次完全不同实验（24 维方法论空间） ==========');
  console.log('1/6 加载数据与理论基准...');
  const bars = loadBars(CODE);
  console.log(`Y0 K线 ${bars.length} 根（${bars[0]?.date} ~ ${bars[bars.length - 1]?.date}）`);
  ...
  console.log('3/6 运行基线（Y0 豆油寻优配方）...');
  // Y0 豆油生产参数（与 RU0 同型：快趋势 + 3x10 熔断 + atr2xClear 过滤 + 5pct 日亏损）
  // 来源: longOptParams Y0 2.85/3.98/39, shortOptParams Y0 2.85/3.98/39
  const baseRecipe: Record<string, number | string | boolean> = {
    ...
    stopAtrMult: 2.3, targetAtrMult: 5.03, maxHoldDays: 39, minRR: 1.0,
